patterns:
    ...
    $potentialClientService = ( * { @Request * на * подключен* * [@AllServices] } *
        ...
        | * { подключени* * @SynonymsForAgreement } *
        | * { подключени* * услуг* } *
        ...
        | * { установ* * @AllServices } *
        | * { устанав* * @AllServices } *
        | * { сколько * установ* * @AllServices } *
        | * { сколько * устанав* * @AllServices } *
        | * сколько * { *плат* * установ* * @AllServices } *
        | * сколько * { *плат* * устанав* * @AllServices } *
        | * сколько * { стоит * установ* * @AllServices } *
        | * сколько * { стоит * устанав* * @AllServices } *
        ...
        | * { @Need * [бы] * подключить* } *
        | * { подключить @Internet } * [переехал* $weight<-1>]
        | * { подключить к @Internet } * [переехал* $weight<-1>]
        | * { подключить к @Television } *
        | * { подключить к услугам } *
        | * { подключение @Internet } * [переехал* $weight<-1>]
    ...
    $commonQuestion = ( @DomRu
        ...
        | * { @Need * информац* } *
        | * { @Need * подробност* } *
        | [$oneWord] [$oneWord] { (помоги*/проконсультир*/@Know) [$pls] } [$oneWord] [$oneWord]
        | * { @Need * подробност* } *
        | * { что ты умеешь } *
        | * { чему тебя *учили } *
        | { (ты/вы/это) * $bot }
    ...
    $serviceCentersAddress = ( * @ServiceCenters * @DayOfWeek *
        ...
        | * { (находит*/находят*/найти) * @ServiceCenters } *
        | * где { (находит*/находят*/найти) * @ServiceCenters } *
        | * где { (вас/вы/[ваш] @ServiceCenters) * (находит*/находят*/найти) } *
    ...
    $otherIntent = ( * { был * звонок * @DomRu } *
        ...
        | * { не согл* * @Payment } * 
        | ~приложение [@DomRu]
        | * { установ* * ~приложение [@DomRu]} * 
        | * { подключ* * ~приложение [@DomRu]} *
        | * { стоит * ~приложение [@DomRu]} * 
        | * { откр* * ~приложение [@DomRu]} *)
    ...
    $praising = ( [ты] [ж] [моя] $regexp<умни(ц|чк)а>
        | [ты] [просто] {[мое/наше] чудо}
        | $regexp<с?пасиб[оа]?> (тебе/вам) * за [твою/вашу] работу
        | хвалю )
    ...
    $noAnswer = [а/ну/эм/хм/м] ( $hello $weight<+0.5>
        | [с] $regexp<добр(ое|ого|ым)> $regexp<утр(ом?|а)>
        | @Allo [@Allo]
        | { @Allo [@Modal] }
        | { [@Modal] [$hello] @Allo [@Allo] } 
        | { (* слыш* *) [$hello] @Allo [@Allo] } 
        | { (* слыш* *) $hello } 
        | (а/эм/ну/хм/м/мм) $hello
        | { девушк* @Allo [@Allo] } 
